...
@pytest.mark.parametrize(
    'config',
    ['addr_lookup_in_app', 'addr_lookup_in_ROM'],
    indirect=True,
)
def test_monitor_addr_lookup(config: str, coverage_run: List[str], dut: Dut) -> None:
    ...
    with open(monitor_log_path, 'w') as log, pexpect.spawn(
        ...
        if config == 'addr_lookup_in_app':
            ...
            p.expect_exact(
                f'Function pointer at {func}; main at {main_addr}; no new line'
            )  # new line should be inserted here and decode is on separate line
